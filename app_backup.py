...
def load_combined_data():
    ...
    # Daftarkan semua kolom finansial dan kolom kunjungan baru agar diproses secara numerik
    numeric_cols = [
        ...
        'Actual Kunjungan Rajal', 'Actual Kunjungan Ranap', 'Actual Kunjungan IGD'
    ]
    ...
    for year, s_name in sheets.items():
        try:
            ...
            # Pemetaan nama kolom alternatif jika di spreadsheet menggunakan format variatif
            for col in df_tmp.columns:
                ...
                if col in ['Actual Kunjungan (IGD)', 'Kunjungan IGD', 'Volume IGD']:
                    df_tmp.rename(columns={col: 'Actual Kunjungan IGD'}, inplace=True)
    ...
    if not df_all.empty:
        ...
        if not df_filtered.empty:
            ...
            if not df_2026.empty:
                ...
                with col1:
                    st.subheader(f"Revenue 2026 ({selected_layanan})")
            ...
            for yr, color in zip(["2025", "2026"], ["#AED6F1", "#2E86C1"]):
                if yr in selected_tahun:
                    ...
                    fig_q_comb.add_trace(go.Bar(x=yr_data['Kuartal'], y=yr_data[actual_rev_column], name=f"Rev {yr}", marker_color=color, offsetgroup=yr))
            ...
            if "2025" in selected_tahun and "2026" in selected_tahun:
                for q in df_q_yoy['Kuartal'].unique():
                    ...
                    v26_r = rows[rows['Tahun'] == '2026'][actual_rev_column].sum()
                    v25_r = rows[rows['Tahun'] == '2025'][actual_rev_column].sum()
            ...
            for yr, color in zip(["2025", "2026"], ["#AED6F1", "#2E86C1"]):
                if yr in selected_tahun:
                    ...
                    fig_m_comb.add_trace(go.Bar(x=yr_data['Bulan'], y=yr_data[actual_rev_column], name=f"Rev {yr}", marker_color=color, offsetgroup=yr))
            ...
            # =====================================================================
            # --- SCRIPT BARU ROW 3: VISUALISASI AKTUAL KUNJUNGAN OPERASIONAL ---
            # =====================================================================
            st.markdown("---")
            st.subheader("📊 Tren Aktual Kunjungan Operasional (Rajal, Ranap, IGD)")
            df_m_kunjungan = df_filtered.groupby(['Bulan', 'Tahun'])[['Actual Kunjungan Rajal', 'Actual Kunjungan Ranap', 'Actual Kunjungan IGD']].sum().reset_index()
            df_m_kunjungan['Bulan'] = pd.Categorical(df_m_kunjungan['Bulan'], categories=month_order, ordered=True)
            df_m_kunjungan = df_m_kunjungan.sort_values(['Bulan', 'Tahun'])

            fig_kunj = go.Figure()
            # Loop untuk tahun-tahun yang dipilih agar grafik dinamis
            for yr in selected_tahun:
                data_yr = df_m_kunjungan[df_m_kunjungan['Tahun'] == yr]
                
                # Gunakan Line + Markers untuk melacak volume kunjungan pasien tiga parameter utama
                fig_kunj.add_trace(go.Scatter(x=data_yr['Bulan'], y=data_yr['Actual Kunjungan Rajal'], name=f"Rajal {yr}", mode='lines+markers', line=dict(width=2)))
                fig_kunj.add_trace(go.Scatter(x=data_yr['Bulan'], y=data_yr['Actual Kunjungan Ranap'], name=f"Ranap {yr}", mode='lines+markers', line=dict(width=2)))
                fig_kunj.add_trace(go.Scatter(x=data_yr['Bulan'], y=data_yr['Actual Kunjungan IGD'], name=f"IGD {yr}", mode='lines+markers', line=dict(width=2)))

            fig_kunj.update_layout(yaxis_tickformat=',.0f', yaxis_title="Jumlah Pasien", template="plotly_white", hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
            st.plotly_chart(fig_kunj, use_container_width=True)

            # =====================================================================
            # --- ROW 4: TREN PER RS & KONTRIBUSI ---
            # =====================================================================
            st.markdown("---")
            ...
            # =====================================================================
            # --- ROW 5: TABEL DETAIL DENGAN PARAMETER BARU ---
            # =====================================================================
            st.markdown("---")
            ...
            # Memasukkan data Aktual Kunjungan operasional ke dalam tabel detail dan sistem export
            df_display = df_table[['Tahun', 'Kuartal', 'Bulan', 'Cabang', actual_rev_column, 'Actual EBITDA', 'EBITDA Margin %', 'Actual Kunjungan Rajal', 'Actual Kunjungan Ranap', 'Actual Kunjungan IGD']].copy()
            ...
            with col_btn1:
                ...
                st.download_button(
                    ...
                    data=excel_data,
                    file_name=f"Performance_Report_Helsa_{selected_layanan}.xlsx",
            ...
            with col_btn2:
                ...
                st.download_button(
                    label="🔵 Export to CSV",
                    data=csv_data,
                    file_name=f"Performance_Report_Helsa_{selected_layanan}.csv",
                    mime="text/csv"
                )
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Actual Kunjungan Rajal": st.column_config.NumberColumn("Kunj. Rajal", format="%,.0f"),
                    "Actual Kunjungan Ranap": st.column_config.NumberColumn("Kunj. Ranap", format="%,.0f"),
                    "Actual Kunjungan IGD": st.column_config.NumberColumn("Kunj. IGD", format="%,.0f")
                }
            )
